...
Кластер kind создаётся заново в каждом прогоне CD, поэтому автоматический деплой после
мержа - это всегда первый релиз, и все сервисы поднимаются как `blue`. Чтобы получить
именно **переключение**, нужен второй релиз против кластера, который уже обслуживает
трафик. Оба сценария запускаются вручную через `workflow_dispatch`:

| Запуск | Что делает CD | Ожидаемый итог |
|---|---|---|
| `fail_mode` выключен | после первого релиза выкатывает orders-service повторно | трафик переезжает `blue` в `green` |
| `fail_mode` включён | подсовывает orders-service заведомо сломанный образ | e2e падают, цвет **не меняется** |

Оба прогона зелёные, и во втором случае это не противоречие: шаг с `fail_mode`
устроен как проверка отката. Он ожидает, что `deploy.sh` завершится с ошибкой,
перехватывает её и сравнивает активный цвет до и после. Сборка падает в обратной
ситуации - если сломанный релиз приняли или трафик всё же переехал на него.
