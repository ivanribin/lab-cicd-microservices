...
|---|---|---|
| `fail_mode` выключен | после первого релиза выкатывает orders-service повторно | трафик переезжает `blue` в `green`, прогон зелёный |
| `fail_mode` включён | подсовывает orders-service заведомо сломанный образ | e2e падают, цвет **не меняется**, прогон красный |

Красный прогон во втором случае - ожидаемый результат, а не поломка: он и есть
доказательство отката. Оба шага сравнивают активный цвет до и после и валят сборку,
если поведение отличается от заявленного.
